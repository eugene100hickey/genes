...
ripke_snp <- function() {
  ...
  df_brain <- readxl::read_excel(temp_file, sheet = 2, skip = 0) # reads into a dataframe from brain sheet. Column headers are in the first row
  df_brain$`P(eQTL)` <- as.character(df_brain$`P(eQTL)`)
  df_blood <- readxl::read_excel(temp_file, sheet = 3, skip = 0) # reads into a dataframe from blood sheet. Column headers are in the first row
